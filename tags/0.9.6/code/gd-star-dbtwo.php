...
class GDSRDB
{
    ...
    function convert_row($row) {
        ...
        if ($row->visitor_voters == 0) {
            ...
            $count_v = sprintf('[ <a href="./admin.php?page=gdsr-stats&gdsr=log&pid=%s&vt=article&vg=visitors"> <strong style="color: red;">%s</strong> </a> ] ', $row->pid, $row->visitor_voters);
        }
    ...
    function convert_comment_row($row) {
        if ($row->visitor_voters == 0) {
            ...
            $visitor_rating =  @number_format($row->visitor_votes / $row->visitor_voters, 1);
            $votes_v = '<strong><span style="color: red">'.$visitor_rating.'</span></strong>';
            $count_v = sprintf('[ <a href="./admin.php?page=gdsr-stats&gdsr=log&pid=%s&vt=comment&vg=visitors"> <strong style="color: red;">%s</strong> </a> ] ', $row->comment_id, $row->visitor_voters);
        }
        ...
        if ($row->user_voters == 0) {
            ...
            $user_rating =  @number_format($row->user_votes / $row->user_voters, 1);
            $votes_u = '<strong><span style="color: red">'.$user_rating.'</span></strong>';
            $count_u = sprintf('[ <a href="./admin.php?page=gdsr-stats&gdsr=log&pid=%s&vt=comment&vg=users"> <strong style="color: red;">%s</strong> </a> ] ', $row->comment_id, $row->user_voters);
        }
