...
class GDSRDatabase
{
    ...
    function get_widget($widget) {
        ...
        if ($widget["select"] != "" && $widget["select"] != "postpage") 
            $where.= " and post_type = '".$widget["select"]."'";
        
        if ($widget["hide_empty"] == "1") {
            if ($widget["show"] == "total") $where.= " and (d.user_votes + d.visitor_votes) > 0";
            if ($widget["show"] == "visitors") $where.= " and d.visitor_votes > 0";
            if ($widget["show"] == "users") $where.= " and d.user_votes > 0";
        }
        if ($widget["hide_noreview"] == "1") $where.= " and d.review > -1";
        ...
        if ($widget["publish_date"] == "range") {
            $where.= " and post_date >+ '".$widget["publish_range_from"]."' and post_date <= '".$widget["publish_range_to"]."'";
        }
        else if ($widget["publish_date"] == "month") {
            $month = $widget["publish_month"];
            if ($month != "" && $month != "0") {
                $where.= " and year(p.post_date) = ".substr($month, 0, 4);
                $where.= " and month(p.post_date) = ".substr($month, 4, 2);
            }
        }
        else if ($widget["publish_date"] == "lastd") {
            if ($widget["publish_days"] > 0)
                $where.= " and TO_DAYS(CURDATE()) - ".$widget["publish_days"]." <= TO_DAYS(p.post_date)";
        }

        if ($cats != "" && $cats != "0")
            $sql = sprintf("SELECT  p.id as post_id, p.post_title, p.post_type, p.post_date, d.* FROM %sterm_taxonomy t, %sterm_relationships r, %sposts p, %sgdsr_data_article d %s and d.post_id = p.id and t.term_taxonomy_id = r.term_taxonomy_id and r.object_id = p.id and t.term_id = %s order by %s %s limit 0, %s",
                $table_prefix, $table_prefix, $table_prefix, $table_prefix, $where, $cats, $col, $sort, $widget["rows"]
            );
        else
            $sql = sprintf("select p.id as post_id, p.post_title, p.post_type, p.post_date, d.* from %sposts p left join %sgdsr_data_article d on p.id = d.post_id %s order by %s %s limit 0, %s",
                $table_prefix, $table_prefix, $where, $col, $sort, $widget["rows"]
            );
